...
export default function PortalFeedbackPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (feedback.trim().length > 5000) {
      toast({ title: "Feedback too long", description: "Please keep your feedback under 5000 characters.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      const { data, error } = await supabase.functions.invoke("process-feedback", { body: { feedback: feedback.trim() } });
      if (error) throw new Error(error.message || "Something went wrong.");
      if (data?.error) {
        toast({ title: "Unable to submit", description: data.error, variant: "destructive" });
        return;
      }
      toast({ title: "Thank you!", description: "Your feedback has been submitted successfully." });
      setFeedback("");
    } catch {
      toast({ title: "Something went wrong", description: "There was an issue submitting your feedback. Please try again later.", variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="relative min-h-screen">
      ...
      <main className="relative z-10 container mx-auto px-4 py-8 pt-24">
        <div className="max-w-2xl mx-auto">
          <Button variant="ghost" className="mb-6 -ml-2" onClick={() => router.push("/portal")}>
          ...
          <div className="bg-card rounded-2xl border border-border p-6 md:p-8">
            ...
            <div className="space-y-4">
              <Textarea placeholder="Tell us what you think..." value={feedback} onChange={(e) => setFeedback(e.target.value)} className="min-h-[160px] resize-y" maxLength={5000} />
              <div className="flex items-center justify-between">
                <p className="text-xs text-muted-foreground">{feedback.length}/5000 characters</p>
                <Button className="text-white" style={{ backgroundColor: '#5022f6' }} onClick={handleSubmit} disabled={submitting || !feedback.trim()}>
